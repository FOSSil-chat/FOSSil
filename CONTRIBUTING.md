# Contributing
- All variable names and filenames must be in **snake_case**
- All constants and statics must be in **SCREAMING_SNAKE_CASE**
- All structs, enums, enum variants, traits, and type aliases must be in **PascalCase**
- All generic type parameters must be in **single uppercase letters** (e.g. T, U, K, V)
- All lifetimes must be **short lowercase names** (e.g. 'a, 'ctx)
- Run `cargo fmt` and `cargo clippy` before submitting - `cargo clippy` must pass without issue.
- All code must have appropriate tests in [tests/](https://codeberg.org/FOSSil/FOSSil/src/branch/main/tests)
